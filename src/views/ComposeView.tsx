...
export const ComposeView: React.FC = () => {
  ...
  const handleGenerateLeadSheet = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        const diagnostics = readLeadSheetFailureDiagnostics(data);
        if (diagnostics) setLeadSheetDiagnostics(diagnostics);
        const error:any=new Error(data.error?.message || data.error || 'Lỗi tạo bản nhạc');
